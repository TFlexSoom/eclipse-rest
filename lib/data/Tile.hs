...
type Rotation = Int.Int8 -- Integer from [0, 5] representing 60 degree rotations. 0 == 0deg and 5 = 300deg
...
class TileMap a where
  ...
  getTileAt :: a -> (Int.Int8, Int.Int8) -> Maybe.Maybe Tile
  getAllTiles :: a -> [Tile]

  drawTile :: a -> Misc.TileDegree -> (a, Tile)
  placeTile :: a -> (Tile, Rotation) -> (Int.Int8, Int.Int8) -> a

data TileMapImpl = TileMapImpl
  { stacks :: Map.Map Misc.TileDegree [Tile],
    placements :: Map.Map (Int.Int8, Int.Int8) TileId,
...
placeTileImpl :: TileMapImpl -> (Tile, Rotation) -> (Int.Int8, Int.Int8) -> TileMapImpl
...
getStartCoords :: [Tile] -> [((Int.Int8, Int.Int8), Int.Int8, Tile)]
getStartCoords ([]) = []
...
newTileMap stacks center homeSystems =
  TileMapImpl
    { stacks = stacks,
      placements = Map.fromList coordsWithId,
      graph = Map.fromList $ zip (take (length rotatedTiles) idGen) rotatedTiles,
      center = center,
      homeSystems = homeSystems
    }
  where
    idGen = iterate (\x -> x + 1) 0
    coordsAndRotation = getStartCoords $ center : (Map.elems homeSystems)
    coordsWithId = map (\(coord, rot, Tile {uniqueId = uniqueId}) -> (coord, uniqueId)) coordsAndRotation
    rotatedTiles = map (\(_, rot, tile) -> applyRot rot tile) coordsAndRotation